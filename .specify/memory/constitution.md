...
Terraform, Ansible e artefatos de infraestrutura são fonte de verdade. Alterações manuais devem ser capturadas como código e documentadas antes de serem consideradas válidas.

### III. Segurança em Primeiro Lugar
Cada mudança deve evidenciar impacto em segurança, isolamento e cadeia de restauração. Nenhum ajuste é aceito se reduzir controles de segurança sem mitigação referenciada.
...
- Sempre usar Wazuh na versão 4.14 em todas as configurações, exceto se o usuário solicitar explicitamente outra versão.
- A aplicação não deve mencionar nomes comerciais de ferramentas (Wazuh, Zabbix, Shuffle, etc.) na interface. Usar termos genéricos: SIEM para Wazuh, Monitoring para Zabbix, SOAR para Shuffle. Toda a interface deve estar em inglês (English first).
