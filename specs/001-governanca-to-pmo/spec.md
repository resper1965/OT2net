...
- **FR-025**: System MUST track costs of Gemini Pro API calls, calculating cost per call based on input/output tokens, presenting dashboard of IA costs (daily, weekly, monthly, by functionality, by phase) with configurable limits and alerts.
...
- **SC-014**: Custo de chamadas à Gemini Pro API permanece abaixo de 300 dólares por mês para processamento típico de um projeto (entrevistas, análises, geração de relatórios).
